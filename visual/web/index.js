...
async function topTopicsReq(dateRange = 7) {
	const response = await fetch(
		BASE_URL + apis.topTopics + `?date_range=${dateRange}`
	)
	return await response.json()
}

async function topTopicsByMonthReq() {
	const response = await fetch(BASE_URL + apis.topTopicsByMonth)
	return await response.json()
}

async function statisticReq() {
	const response = await fetch(BASE_URL + apis.statistic)
	return await response.json()
}

async function biasAndSentimentStatisticReq() {
	const response = await fetch(BASE_URL + apis.biasAndSentimentStatistic)
	return await response.json()
}

async function newsByStateReq() {
	const response = await fetch(BASE_URL + apis.newsByState)
	return await response.json()
}
...
const app = createApp({
	data() {
		return {
			dateRanges: [7, 30, 180, 365],
			data: [],
			topTopicsByMonth: null,
			currentYearMonth: '',
			timer1: null, //
			requested: false,
			chartInst1: null,
	...
	methods: {
		...
		async getTopTopicsByMonth() {
			...
			this.topTopicsByMonth = data
			this.currentYearMonth = Object.keys(data)[0]
			this.makeChart3()
			this.makeChart5()

			this.timer1 = setInterval(() => {
				const keys = Object.keys(this.topTopicsByMonth)
				const index = keys.indexOf(this.currentYearMonth)
				if (index === keys.length - 1) {
					this.currentYearMonth = keys[0]
				} else {
					this.currentYearMonth = keys[index + 1]
				}
				this.makeChart3()
			}, 3000)
		},
		async getStatistic() {
			let data = await statisticReq()
			this.statistic = data
		},
		async getBiasAndSentimentStatistic() {
			let data = await biasAndSentimentStatisticReq()
			this.biasAndSentimentStatistic = data.slice(0, 5)
			this.activeBSMedia = data[0][0]
			this.makeChart2()
		},
		async getNewsByState() {
			let data = await newsByStateReq()
			this.mapData = USA_STATES.map(state => {
				const d = data[state]
				return {
					name: state,
					data: d ?? {news_count: 0, topics: []},
					value: d?.news_count || 0,
				}
			})
			this.makeMapChart()
		},
		async getTopTopicsLastYear() {
			let data = await topTopicsReq(365)
		...
		onChangeActiveBSMedia(e) {
			...
			this.activeBSMedia = v
			this.makeChart2()
		},
		onChangeCurrentBiasOrSentiment() {
			this.currentBiasOrSentiment =
				this.currentBiasOrSentiment === 'bias' ? 'sentiment' : 'bias'
			this.makeChart2()
		},
		onChangeTopTopicsDateRange(e) {
			const v = e.target.value
		...
		makeChart1() {
			if (!this.topTopics.length) {
				this.chartInst1?.dispose()
				this.chartInst1 = null
				alert('no data')
				return
			}
			this.chartInst1 =
				this.chartInst1 || echarts.init(document.getElementById('chart_1'))
			...
			// 使用刚指定的配置项和数据显示图表。
			this.chartInst1.setOption(option)
		},
		makeChart2() {
			this.chartInst2 =
				this.chartInst2 || echarts.init(document.getElementById('chart_2'))

			const data = this.biasAndSentimentStatistic
			const type = this.currentBiasOrSentiment
			const media = this.activeBSMedia
				...
				series: [
					{
						...
						type: 'pie',
						radius: '60%',
						data: Object.entries(data.find(c => c[0] === media)[1][type]).map(
			...
			// 使用刚指定的配置项和数据显示图表。
			this.chartInst2.setOption(option)
		},
		makeChart3() {
			this.chartInst3 =
				this.chartInst3 || echarts.init(document.getElementById('chart_3'))

			const data = (this.topTopicsByMonth?.[this.currentYearMonth] ?? [])
				.slice(0, 10)
				.reverse()

			// 指定图表的配置项和数据
			var option = {
				color: ['rgb(111,238,239)'],
				grid: {
					left: 90,
					bottom: 20,
					top: 10,
			...
			// 使用刚指定的配置项和数据显示图表。
			this.chartInst3.setOption(option)
		},
		makeChart4() {
			const container = document.getElementById('chart_4')
			const { offsetWidth, offsetHeight } = container

			const data = this.topTopicsLastYear.slice(0,20).map(c => ({
				label: c.name,
				Frequency: c.count,
			}))

			const viz = new BubbleChart({
				container: '#chart_4',
				label: 'label',
				size: 'Frequency',
				width: offsetWidth,
		...
		makeMapChart() {
			this.chartMapInst =
				this.chartMapInst || echarts.init(document.getElementById('map'))
			...
			option = {
				tooltip: {
					...
					formatter: function (params, ticket, callback) {
						const {name, data: {news_count, topics}} = params.data;
						html = `<h3 style="margin: 0 0 10px" class="flex between"><span>${name}</span><span style="margin-left: 12px;color:rgb(10,34,165)">${news_count}</span></h3>`

						topics.forEach(topic => {
							html += `<div class="flex between">${topic[0]}&nbsp;&nbsp;&nbsp;&nbsp;<b>${topic[1]}</b></div>`
						})
			...
			this.chartMapInst.setOption(option)
		},
		makeChart5() {
			if (!this.topTopicsLastYear.length || !this.topTopicsByMonth)
				return

			this.chartInst5 =
				this.chartInst5 || echarts.init(document.getElementById('chart_5'))

			const topTopics = this.topTopicsLastYear
				.map(c => c.name)
				.slice(0, 6)
			const months = Object.keys(this.topTopicsByMonth)
			const data = topTopics.map(topic => {
				return {
					type: 'line',
					name: topic,
					data: Object.values(this.topTopicsByMonth).map(c => {
						const item = c.find(c => c.name === topic)
			...
			// 使用刚指定的配置项和数据显示图表。
			this.chartInst5.setOption(option)
		},
		makeChart6() {
			this.chartInst6 =
			...
			// 使用刚指定的配置项和数据显示图表。
			this.chartInst6.setOption(option)
		},
	},
	mounted() {
		this.getTopTopics()
		this.getTopTopicsLastYear()
		this.getTopTopicsByMonth()
		this.getStatistic()
